...
Codex already uses progressive disclosure for skills. It initially provides the model with each available skill's name, description, and path, then loads the full `SKILL.md` only when a skill is selected. The initial list is capped at 2% of the model's context window, or 8,000 characters when the context window is unknown. If the catalog exceeds that budget, Codex shortens descriptions and may omit skills.
...
This proposal adds one more level of progressive disclosure. Codex would keep the complete skill catalog in a host-side index, retrieve a small candidate set for each task, and expose only those candidates to the model. Explicit `$skill-name` invocation and autocomplete would resolve against the same index without requiring the complete catalog in model context. Once Codex selects a skill, the existing full `SKILL.md` loading behavior would remain unchanged.
...
In one real Codex installation, a local inventory utility found 241 candidate skill entries across user skill directories and plugin caches. The raw names, descriptions, and paths represented approximately 92,000 characters, or roughly 23,000 tokens using a simple four-characters-per-token estimate. The complete skill bodies occupied approximately 2.61 MB, but those bodies were not part of the initial prompt.

The 241-entry figure is an inventory upper bound, not an exact count of active skills. A naive scan of the complete plugin cache can include old plugin versions and duplicates. That distinction exposes another requirement: a reliable discovery mechanism must index active plugin versions and enabled skills, not every `SKILL.md` found under a cache directory.

For a model with a 400,000-token context window, the documented 2% catalog limit is approximately 8,000 tokens. This prevents unbounded growth, but 8,000 tokens is still substantial for routing metadata. More importantly, the cap means the model may never see some installed skills. Adding capabilities can therefore make existing capabilities harder to discover.
...
A user-authored router skill cannot resolve this tradeoff. Codex builds the available-skills catalog before deciding whether to invoke that router. The router therefore runs after the metadata cost has already been paid. If skills are omitted because of the catalog budget, the router itself can also be omitted. Disabling other skills reduces the catalog, but then those skills lose native implicit invocation and `$`-mention usability.
...
The index could be in memory, SQLite backed, or integrated with existing Codex state. The storage choice is less important than its behavior. It should be local, fast, invalidated when skill files or plugin state change, and rebuilt safely if it becomes unavailable.

The indexer should parse only metadata. It must not execute scripts, install dependencies, or load referenced files. It should apply file-size and parsing limits, canonicalize paths, and follow the same trust rules as current skill loading.

### 2. Resolve explicit invocation without model discovery

When a user selects or types `$skill-name`, the client already knows the intended skill. The client should resolve that name directly against the local index and attach the selected skill to the request. The complete catalog does not need to be shown to the model for this path.
...
If no candidate reaches a minimum confidence threshold, or if the model says the candidates do not fit, Codex should support a second search with a rewritten query, a larger result set, or a category filter. A final fallback can use the existing bounded catalog behavior.

This recovery path matters more than choosing a sophisticated ranking algorithm on day one. A simple retriever with visible uncertainty and widening is safer than an opaque retriever that always returns a confident answer.
...
File watching can provide fast updates, with a content-hash scan as a fallback. Installers and plugin managers can also emit an explicit invalidation event. A newly installed skill should become available to autocomplete and implicit routing without requiring the user to edit a manifest.
...
Duplicate handling should use qualified names, source scope, plugin activation state, and precedence rules. If two active skills legitimately share a name, the picker should disambiguate them and implicit search should retain source information for the model.
...
For a catalog with more than 200 skills, this should reduce initial skill metadata by well over 90% in typical turns. Exact savings should be measured from the rendered prompt rather than estimated from file size.

Prompt caching is helpful but does not replace this change. Caching can reduce latency and input cost for repeated prefixes, while the cached content still occupies the model context and counts toward applicable rate limits. Deferred discovery reduces the input itself.
...
Indexing must not execute any code. The loader should retain existing approval, sandbox, dependency, and trust checks. Canonical path validation is necessary so a crafted skill cannot make the index read arbitrary files through path traversal or an unexpected symlink.
...
Add a feature flag that enables the local index and candidate retrieval while retaining the current catalog as a fallback. Compare routing outcomes and prompt use through controlled evaluation and opt-in telemetry.
...
Use deferred discovery by default when the full skill catalog would consume a meaningful portion of its budget or require omission. Smaller catalogs can continue using the simpler current path if it performs better.
...
Codex already supports per-skill enablement. This works when users know in advance which skills a project needs. It does not preserve universal availability across projects, and it adds maintenance when skills are installed or project needs change.

### Add a user-authored router skill

A router skill can search files after invocation, but it cannot remove the catalog that Codex injected before selecting the router. Hiding leaf skills in a separate directory can save tokens, but it also removes native autocomplete and implicit invocation unless the client participates.

### Bundle many skills into domain meta-skills

An AWS, frontend, or document meta-skill can reduce catalog entries. This requires manual packaging, introduces another authoring convention, and does not generalize cleanly to unrelated or newly installed skills.
...
A community plugin can prototype search and loading, but it cannot fully preserve native skill-picker behavior or guarantee that its router is invoked before catalog construction. A first-class implementation can.
...
4. If a first-party implementation is not planned, could Codex expose a supported pre-context skill-discovery hook and a client-side skill-index API so the community can implement it without losing native usability?
